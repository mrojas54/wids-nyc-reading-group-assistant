...
**in numeric order** (`001` → `022`). They are hand-applied — paste each file
...
| `022_papers_prerequisites.sql` | Adds nullable `papers.prerequisites` JSONB for the editable new-paper announcement bundle (`lede`, linked or plain `items`, summary, short title, and draft metadata). |
...
- Portal columns and helpers exist: `members.auth_user_id`, `members.role`
  ...
  `papers.zotero_item_key`, `papers.prerequisites`, `availability.created_at`, the
