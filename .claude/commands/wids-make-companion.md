---
description: "[DEPRECATED — superseded by Paper Pal] Generate static companion page (Mermaid walkthrough + Colab notebook) for a paper."
---

# /wids-make-companion — DEPRECATED

> **Superseded by Paper Pal.** The static-companion flow (Mermaid walkthrough + Colab notebook + JSON fixture under `web/content/papers/`) has been replaced by Paper Pal, the interactive synthesis surface at `/papers/<id>` in the member portal. Companion payloads now live in the `paper_companions` table (migration `013_paper_companions.sql`) and are generated by the operator via the `/new` route in the portal, not by this slash command.
>
> This command is kept for rollback only and **should not be run in production**. New companions: open the portal as an operator, click "Generate companion" on the paper's screen (or visit `/new`), upload the PDF, save. The result populates `paper_companions` and updates `papers.companion_url` automatically.
>
