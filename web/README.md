...
| `/papers/[id]` | Public Paper Pal reading page — serif Newsreader prose with Mermaid diagrams and code blocks. No auth is required to read existing content. If a paper exists in the catalog but has no synthesized content yet, only the operator/admin or a meeting leader for that paper sees the synthesize CTA; others see a read-only empty state. |
