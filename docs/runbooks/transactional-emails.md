...
| `availability-reminder.{html,txt}` | `scheduled_tasks/availability-chase.md` Step 5c; `scripts/gmail_raw_drafts.py reminder-manifest` for the raw-MIME path | Live only after the operator replies `remind`. **Drafted** one recipient at a time for active members without an `availability` row — operator sends. Flow: [`docs/availability-reminder-flow.md`](../availability-reminder-flow.md). Draft rows log `needs_action` (amber on `/admin/logs`). Lede ("It's been too long") is for a lapsed regular — for a brand-new member use `welcome-availability` instead. Note the location-chip marker in the HTML is longer than the chase spec quotes it (`BEGIN-OPTIONAL-CHIP: location \| OMIT …`); a matcher built on the quoted form silently mails an empty chip. Carries a **block pair**, `paper` / `paper_pending`, resolved from `meetings.paper_id` by `resolve_blocks()` before substitution (chase Step 5c.1a): `paper_pending` is the "Paper Pal coming soon" state for a cycle whose leader has not chosen yet, and references no `paper.*` token. `render_email_previews.py` previews both states. |
